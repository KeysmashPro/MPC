...
time_data times = {0.0f, 0.0f, 0.0f, 0ULL};
vk_context ctx;
...
void draw_frame()
{
    vkWaitForFences(ctx.device, 1, &ctx.inFlightFences[times.frame], VK_TRUE, UINT64_MAX);
    vkResetFences(ctx.device, 1, &ctx.inFlightFences[times.frame]);
    ...
    VkResult result = vkAcquireNextImageKHR(ctx.device, ctx.swapChain, UINT64_MAX, 
                                            ctx.imageAvailableSemaphores[times.frame], 
    ...
    vkResetCommandBuffer(ctx.commandBuffers[times.frame], 0);
    record_command_buffer(ctx.commandBuffers[times.frame], imageIndex);
    
    VkSubmitInfo submitInfo = {};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
    
    VkSemaphore waitSemaphores[] = {ctx.imageAvailableSemaphores[times.frame]};
    ...
    submitInfo.pCommandBuffers = &ctx.commandBuffers[times.frame];
    
    VkSemaphore signalSemaphores[] = {ctx.renderFinishedSemaphores[times.frame]};
    submitInfo.signalSemaphoreCount = 1;
    submitInfo.pSignalSemaphores = signalSemaphores;
    
    vkQueueSubmit(ctx.queue, 1, &submitInfo, ctx.inFlightFences[times.frame]);
